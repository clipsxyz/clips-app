...
type Tab = 'Finglas' | 'Dublin' | 'Ireland' | 'Following' | string;

function NavItem({ path, label, icon, badge }: { path: string; label: string; icon: React.ReactNode; badge?: number }) {
...
function BottomNav() {
  const item = (path: string, label: string, icon: React.ReactNode, badge?: number) => (
    <NavItem key={path} path={path} label={label} icon={icon} badge={badge} />
  );

  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-gray-950/80 backdrop-blur-xl border-t border-gray-200/50 dark:border-gray-800/50 z-40">
      ...
      >
        ...
        {burst && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <svg className="w-20 h-20 text-red-500 animate-ping" fill="currentColor" viewBox="0 0 20 20">
...
function EngagementBar({ post, onLike, onBookmark, onShare, onComment, onReclip }: {
  post: Post;
  onLike: () => Promise<void>;
  onBookmark: () => Promise<void>;
  onShare: () => void;
  ...
  const handleReclip = async () => {
    if (busy) return;
    setBusy(true);
    setReclipped(!reclipped);
    try {
      await onReclip();
    } catch {
      setReclipped(reclipped); // Revert on error
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 dark:border-gray-800">
      <div className="flex items-center gap-4">
        <IconButton
          ...
          className={cn(
            ...
            liked ? "text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20" : "hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20"
          )}
        >
        ...
        <IconButton
          ...
          onClick={onComment}
          aria-label="Comment"
          className="hover-scale transition-all duration-200 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20"
        >
        ...
        <IconButton
          ...
          onClick={onShare}
          aria-label="Share"
          className="hover-scale transition-all duration-200 hover:text-green-500 hover:bg-green-50 dark:hover:bg-green-900/20"
        >
          <FiShare2 size={18} />
        </IconButton>
      </div>
      ...
      <IconButton
        ...
        className={cn(
          "hover-scale transition-all duration-200",
          bookmarked ? "text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-900/20" : "hover:text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-900/20"
        )}
      >
...
function FeedPageWrapper() {
  ...
  async function loadMore() {
    ...
    setLoading(true);
    try {
      const page = await fetchPostsPage(active, cursor);
  ...
  const handleRefresh = async () => {
    ...
    try {
      const page = await fetchPostsPage(active, 0);
      setPages([page.items]);
      setCursor(page.nextCursor);
      setEnd(page.nextCursor === null);
  ...
  const loadingMore = loading && flat.length > 0;
        ...
        >
            ...
            <Card className="mx-4 my-4 p-4 border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/50 animate-fade-in-up">
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 w-8 h-8 bg-red-100 dark:bg-red-900 rounded-full flex items-center justify-center">
                  <FiZap size={16} className="text-red-600 dark:text-red-400" />
                </div>
          ...
          {flat.map(p => (
            <FeedCard
              key={p.id}
              post={p}
              onLike={() => toggleLike(userId, p.id)}
              onBookmark={() => toggleBookmark(userId, p.id)}
              onFollow={() => toggleFollowForPost(userId, p.id)}
              onShare={() => {
                setSharePost(p);
                setShowShare(true);
              }}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addPost({
        ...
        locationLabel: user?.location || user?.regional_location || 'Unknown Location',
        storyLocation: storyLocation || 'Unknown Location',
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <button
        onClick={handleSubmit}
        disabled={isSubmitting || !text.trim()}
        className="w-full px-4 py-3 bg-brand-600 text-white rounded-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed hover:bg-brand-700 transition-colors"
      >
